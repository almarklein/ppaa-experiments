...
vec2 gradientAt(sampler2D sampler, vec2 pos, vec2 sampling)
{
    vec2 stepx = vec2(sampling.x, 0.0);
    vec2 stepy = vec2(0.0, sampling.y);

    /* Explicit Sobel
    float left = rgb2lumi( texture(sampler, pos-stepx) ) * 2.0
               + rgb2lumi( texture(sampler, pos-stepx-stepy) )
               + rgb2lumi( texture(sampler, pos-stepx+stepy) );
    float right = rgb2lumi( texture(sampler, pos+stepx) ) * 2.0
                + rgb2lumi( texture(sampler, pos+stepx-stepy) )
                + rgb2lumi( texture(sampler, pos+stepx+stepy) );

    float top = rgb2lumi( texture(sampler, pos-stepy) ) * 2.0
              + rgb2lumi( texture(sampler, pos-stepy-stepx) )
              + rgb2lumi( texture(sampler, pos-stepy+stepx) );
    float bottom = rgb2lumi( texture(sampler, pos+stepy) ) * 2.0
                 + rgb2lumi( texture(sampler, pos+stepy-stepx) )
                 + rgb2lumi( texture(sampler, pos+stepy+stepx) );
    */
    ...
    float alpha = 0.358;
    float left = rgb2lumi( texture(sampler, pos-stepx - alpha*stepy) )
               + rgb2lumi( texture(sampler, pos-stepx + alpha*stepy) );
    float right = rgb2lumi( texture(sampler, pos+stepx - alpha*stepy) )
                + rgb2lumi( texture(sampler, pos+stepx + alpha*stepy) );

    float top = rgb2lumi( texture(sampler, pos-stepy - alpha*stepx) )
              + rgb2lumi( texture(sampler, pos-stepy + alpha*stepx) );
    float bottom = rgb2lumi( texture(sampler, pos+stepy - alpha*stepx) )
                 + rgb2lumi( texture(sampler, pos+stepy + alpha*stepx) );
    ...
    float left = rgb2lumi( texture(sampler, pos - a3*stepx) ) * a4
               + rgb2lumi( texture(sampler, pos - a1*stepx - a2*stepy) )
               + rgb2lumi( texture(sampler, pos - a1*stepx + a2*stepy) );
    float right = rgb2lumi( texture(sampler, pos + a3*stepx) ) * a4
                + rgb2lumi( texture(sampler, pos + a1*stepx - a2*stepy) )
                + rgb2lumi( texture(sampler, pos + a1*stepx + a2*stepy) );
    float top = rgb2lumi( texture(sampler, pos - a3*stepy) ) * a4
                 + rgb2lumi( texture(sampler, pos - a1*stepy - a2*stepx) )
                 + rgb2lumi( texture(sampler, pos - a1*stepy + a2*stepx) );
    float bottom = rgb2lumi( texture(sampler, pos + a3*stepy) ) * a4
                 + rgb2lumi( texture(sampler, pos + a1*stepy - a2*stepx) )
                 + rgb2lumi( texture(sampler, pos + a1*stepy + a2*stepx) );
    */
...
vec4 aa_shader(sampler2D sampler, vec2 fragcoord) {

    //vec2 resolution = vec2(300,300);
    vec2 resolution = vec2(textureSize(sampler, 0).xy);
    vec2 sampling = 1.0/ resolution;
    vec2 stepx = vec2(sampling.x, 0.0);
    vec2 stepy = vec2(0.0, sampling.y);
    ...
    // Calculate gradient
    vec2 grad = gradientAt(sampler, pos, sampling);
    ...
    if ( gm < 0.05) {
        vec3 result = texture(sampler, pos).rgb;
    ...
    grad += gradientAt(sampler, pos -stepx, sampling) * alpha1;
    grad += gradientAt(sampler, pos +stepx, sampling) * alpha1;
    grad += gradientAt(sampler, pos -stepy, sampling) * alpha1;
    grad += gradientAt(sampler, pos +stepy, sampling) * alpha1;
    grad += gradientAt(sampler, pos -stepx -stepy, sampling) * alpha2;
    grad += gradientAt(sampler, pos -stepx +stepy, sampling) * alpha2;
    grad += gradientAt(sampler, pos +stepx -stepy, sampling) * alpha2;
    grad += gradientAt(sampler, pos +stepx +stepy, sampling) * alpha2;
    ...
    vec3 result = texture(sampler, pos).rgb * (1.0-strength);
    ...
    N = extend * N;
    result += texture(sampler, pos+N*sampling).rgb * strength * 0.5;
    result += texture(sampler, pos-N*sampling).rgb * strength * 0.5;
    //result += texture(sampler, pos+N*sampling).rgb * strength * 0.333;
    //result += texture(sampler, pos-N*sampling).rgb * strength * 0.333;
    //result += texture(sampler, pos+N*2.0*sampling).rgb * strength * 0.16666;
    //result += texture(sampler, pos-N*2.0*sampling).rgb * strength * 0.16666;
