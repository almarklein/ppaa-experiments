...
filter. However, in real-time rendering, the larger number of pixels impacts
fragment shader performance, as well as memory usage. This makes SSAA with a
large scale-factor impractical, while smaller factors (e.g. $<= 2$) are less
...
Therefore, MSAA is of limited use in scientific applications.
...
of individual pixels, mostly at edges/boundaries in the image. Although PPAA
algorithms are not a recent development, their advantages are still appealing:
it is relatively easy to implement as a post processing step (without affecting
the rendering pipelines or shaders) and is usually more performant than MSAA and
SSAA.

In the present work, a novel PPAA algorithm is presented, with several benefits
compared to existing methods.

One major downside of PPAA is that it is applied once the aliasing has already
occurred, so that artifacts can not always be \textit{removed}, but rather their
...
one pixel to another. In the present work we also discuss how PPAA can be
combined with SSAA (with a small scale-factor) to counter these effects.
...
\end{tabular}
\end{center}
\caption{Overview of the different algorithms compared in this paper.
...
consistent diffusion, the preservation of small structures, the combination of directional-diffusion and edge-search,
...
taken that lie on opposite sides of the current pixel-center. A larger distance between the samples results in more
...
In effect, the proposed algorithm employs two 'blurring' mechanisms: the diffusion
...
Since the maximum search distance must be bounded, the algorithm
...
without losing too much performance. Many derived algorithms, including AXAA
...
not severe. However in synthetic images (e.g. scientific plots) such cases
...
The following subsections describe the experiments that were performed to evaluate the proposed algorithm in terms of quantitative
...
\begin{figure}[tb]
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    ...
    \caption{sponza.png}
  \end{subfigure}
  \caption{The four test images used in the experiments.}
...
result of e.g. an 8x SSAA method. However, this approach was found to favor
blurry images, making the quantitative errors less representative of perceived
visual quality.
...
the peak signal-to-noise ratio (PSNR) was calculated from that image and the
reference. The resulting values indicate how well each image can be reconstructed
to match the reference image.

This approach still favors blurriness, but much less than comparing the images at their original resolution.
The 'blur' method was included for comparison, which performs a Gaussian blur with sigma 0.6.
...
together, but it can be seen that \texttt{ssaax4} and \texttt{ssaax2} consistently perform
the best, while the \texttt{blur} method acts as a baseline for a "poor" result.

The \texttt{fxaa} methods barely outperform the \texttt{blur} method; although they are less blurry, they also introduce more artifacts.
The \texttt{ddaa} methods produce significantly better results, especially for the 'line' and 'plot' images.

Interestingly, the \texttt{ddaa1} method has a smaller error than \texttt{ddaa2} on most
images. It appears that PSNR does not adequately capture the perceptual advantages of the
edge search performed in \texttt{fxaa3d} and \texttt{ddaa2}; to properly evaluate the proposed algorithm, we also need thorough
...
\begin{figure}[tb]
  ...
  The grids look clean for \texttt{ddaa2} while \texttt{fxaa3d} shows artifacts.
  }
...
\begin{figure}[tb]
  ...
  It can be seen that \texttt{ddaa2} shows clean text, crisp circles, and does not have the dotted artifacts that the \texttt{fxaa} algorithms suffer from because they skip pixels.
  }
...
\begin{figure}[tb]
  ...
  It can be seen that the \texttt{ddaa2} method has the most uniform diffusion (independent of the angle of the line),
  that thin lines are not diminished.
  }
  \label{fig:compare8_circles}
\end{figure}

\subsubsection{Methods}

The goal of a PPAA algorithm is to reduce the effects of aliasing such that the
resulting image is visually pleasing. To investigate whether this is successful,
a selection of negative effects that should be reduced and/or avoided are
listed. The visual results are then compared against this list.
...
The \texttt{ddaa} algorithms look better regarding all these aspects.

In Figure~\ref{fig:compare8_circles}, a few coloured circles are depicted. The
figure shows that going around a circle, the \texttt{ddaa2} method has the most
uniform amount of blur. It also looks closest to the \texttt{ssaax4} (which can
be considered a reference). In contrast, \texttt{fxaa3c} is more similar to the
\texttt{blur} method and \texttt{fxaa3d} diminishes thin lines. It can be seen
that the green circle is not smoothed by any of the PPAA methods. This is because
these methods convert RGB colors to a scalar luma value, and the luma of certain
colors is so close to the luma of the background, that the contrast falls below
the algorithms threshold. This is an intrinsic limitation of most PPAA methods,
which is not easy to overcome due to limited availability of memory registers in a
shader program.
...
\begin{figure}[tb]
  ...
  \caption{The image used to study temporal effects. The actual image is an animated png: the top-left cross rotates around its center, the top-right
  circles rotate around an off-center point, and the vertical lines slant back and forth.
  See the \href{https://almarklein.github.io/ppaa-experiments/viewer.html\#image=animated.png}{web viewer} for the animated version.
  }
  \label{fig:animated}
\end{figure}
...
targets use cases where only a single frame is available, or where there is not
...
which is played at 25 FPS, stored in an animated PNG file
...
The animations can be visually compared in the interactive \href{https://almarklein.github.io/ppaa-experiments/viewer.html}{web viewer} (see Supplemental Materials), where
...
occurring as the part of a line moves to the neighbouring pixel, also known as 'shimmering' or 'jitter'.
...
using a timestamp query. These 100 times were sorted and the first and last quadrant discarded as outliers.
The mean and standard deviations for the remaining 50 times were displayed. A high standard deviation indicates
that the GPU was likely also doing other work, so the benchmark could be repeated.

The measured times vary between different hardware. To make the results easier
to compare across devices, the measured times were normalized by using the time
for the "blur" algorithm as a baseline. (Since its speed is in the same order
of magnitude as the PPAA methods, using the blur shader as a reference yields
more stable numbers than a shader that only copies the image.)
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=1\textwidth]{images/performance_benchmark_results.png}
  \caption{The performance of the considered PPAA algorithms accross different devices.
    The values represent relative performance (\% of \texttt{blur} baseline). Lower values indicate faster performance.
    The four bars in each group represent the four test images of Figure~\ref{fig:the4images} (lines, circles, plot, sponza).
    The code to run the experiments, as well as the raw numbers are available at https://github.com/almarklein/ppaa-experiments.
  }
  \label{fig:performance_benchmark_results}
\end{figure}

The final results are shown in Figure~\ref{fig:performance_benchmark_results}. The values for \texttt{blur} are all 100\%, since it is used as the baseline.

The \texttt{ssaax2} shader is not a PPAA algorithm, but included for comparison.
This shader is a cubic Mitchel filter, optimized to take just 12 texture
samples. Note that this only measures the resolve step; the costs for rendering
the frame at a higher resolution are not taken into account. These costs are
significant and depend on various factors, such as the scene being rendered and
the shaders being used. That said, these costs are also incurred when a HiDPI
screen is used.
...
Thanks to its edge-search it produces better results for long-edge artifacts than \texttt{ssaax2}.

Although it is capable of softening temporal artifacts somewhat, motion artifacts
can be considered its main weakness (as for all PPAA methods).
...
at twice the resolution, which is similar to what happens with \texttt{ssaax2}.

This observation was the inspiration for an approach that was adopted in the PyGfx render
...
independent on the kind of screen being used, and yields a comparable quality.

To avoid complexity, this approach was not included as one of the
...
images it demonstrates a superior quality,  comparable with \texttt{ssaax4}.
...
displays). Using temporal anti-aliasing methods is then preferred to alleviate motion artifacts.
