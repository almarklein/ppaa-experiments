...
\teaser{
  \includegraphics[width=0.8\textwidth]{images/teaser2.png}
...
Quantitative and qualitative results show that DDAA produces better results,
...
filter (see Figure~\ref{fig:ssaa} for an example). However, in real-time
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=\textwidth]{images/circles_ssaa.png}
  \caption{Comparison of SSAA with sampling factors 2 (4x pixels), and 4 (8x pixels).
  Higher sampling factors improve quality, but significantly increase computational cost due to the
  larger pixel count and the need for wide downsampling filters.}
  \label{fig:ssaa}
\end{figure}
...
To mitigate these downsides, PPAA can be combined with SSAA, by rendering at a slightly higher
resolution (e.g. 1.5 or 2.0), then apply PPAA, and then downsample to the final
resolution.
...
shown in Figure~\ref{fig:rear_car}. Improvements to this algorithm have also
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=\textwidth]{images/rear_car.png}
  \caption{Comparison of the proposed algorithm (\texttt{ddaa2}), FXAA 3.11 console (\texttt{fxaa3c}), and
      FXAA 3.11 desktop (\texttt{fxaa3d}) on an image containing near-horizontal lines.
      The algorithms that include edge-search (\texttt{fxaa3d} and \texttt{ddaa2}) produce a smoother result.}
  \label{fig:rear_car}
\end{figure}
...
An experiment was performed, applying different kernels to estimate
the angle of an edge in an image containing a line at a known angle. The measured
angles were compared with the reference angle to calculate the error. The
...
in the \textit{fxaa3c} panel in Figure~\ref{fig:circles_angle}. For this reason, the
proposed algorithm uses the Scharr kernel, resulting in a consistent smoothing, see
the \textit{ddaa1} panel in Figure~\ref{fig:circles_angle}.
...
To perform diffusion (i.e. smoothing) in a particular direction, an elongated kernel
should be used. In practice, a few (bilinearly interpolated) samples are taken that lie on a line.
The proposed method uses just two samples, whose distance is larger if more smoothing is required.
Their maximum distance from the center is such that the
contribution of the center pixel is still the largest. For a horizontal line that distance would be 0.66 pixels.
However, if the diagonal case is considered, an interesting effect occurs. As shown in Figure~\ref{fig:diffuse_samples},
if the sample is taken at the position where the 4 pixels meet, their contribution would be equal, which would cause a
significant amount of blur. To reduce this effect, a maximum distance of 0.5 pixels is used instead (indicated by the dot).
...
\begin{figure}[tb]
  ...
  \caption{Illustration of the sample location for maximum diffusion for a
  diagonal and horizontal edge. The center square represents the pixel being
  evaluated. The blue dots represent a distance of 0.5 pixels from the center. A
  larger value, like $\sqrt{2}$ would for diagonal lines put the sample point at
  the point where 4 pixels meet, resulting in an overly blurry result.}
...
During the edge-search, which is implemented in both FXAA ($fxaa3d$) and DDAA
($ddaa2$), the algorithm walks vertically or horizontally along the edge,
...
In Figure~\ref{fig:sample_skip_artifacts} the effect can be observed. It also
...
samples, reaching 26 pixels. The default configuration of $ddaa2$ reaches 15
pixels.


\begin{figure}[tb]
  \centering
  \includegraphics[width=\textwidth]{images/sample_skip_artifacts.png}
  ...
  \label{fig:sample_skip_artifacts}
\end{figure}
...
The following sections evaluates the proposed algorithm in terms of quantitative
errors, qualitative results, and performance.
...
the result of each method was compared to a reference image.
...
Therefore, each image obtained by the different PPAA methods was upscaled to 8 times the
...
a 'blur' method was introduced for comparison, which performed a Gaussian blur with sigma 0.6.
...
\hline
lines & 17.2 & 16.3 & 15.2 & 15.1 & 14.8 & 15.4 & 15.4 \\
circles & 22.6 & 21.5 & 18.8 & 18.9 & 18.7 & 18.6 & 18.8 \\
plot & 23.6 & 23.5 & 22.4 & 22.7 & 22.7 & 23.7 & 23.2 \\
sponza & 30.2 & 30.0 & 29.2 & 29.1 & 29.2 & 29.2 & 29.2 \\
total & 23.4 & 22.8 & 21.4 & 21.4 & 21.4 & 21.7 & 21.7 \\
\end{tabular}
\end{center}
...
together, but it can be observed how 'ssaax4' and 'ssaax2' consistently perform
the best, while the 'blur' method acts as a baseline for a "poor" result.

The 'fxaa' methods barely outperforms the 'blur' method; although they are less blurry, thet also introduce more artifacts.
The 'ddaa' methods produces significantly better results, especially for the 'plot' example.
The fact that 'ddaa2' performs less good on that example than 'ddaa1' can be explained because the
lines of the axes are blurred in the presense of the tickmars, see Figure~TODO.

Although these results suggest that the propsosed method is an improvement over existing methods, these numbers cannot
show the advanatages of e.g. the edge search performed in 'fxaa3d' and 'ddaa2'; thorough qualitative comparisons are required too.
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=\textwidth]{images/compare8.png}
...
In Figure~\ref{fig:compare8} the proposed algorithm (\texttt{ddaa2}) is compared against other algorithms.
It can be observed that the diagonal lines are less jagged than they are for \texttt{fxaa3d}, but also not
blurry like \texttt{fxaa3c}.

It can also be seen that near-vertical and near-horizontal lines are smooth,
albeit the length of the 'gradient' is slightly shorter than that of
\texttt{fxaa3d}. In Figure~\ref{fig:sample_skip_artifacts} the gradients
can clearly be seen for a vertical line with an obstruction and one line without
an obstruction. It shows that the proposed algorithm has a smooth gradient
without artifacts. The gradient for \texttt{fxaa3d} is longer, but shows steps, and
has gaps around the obstruction. This illustrates how the proposed algorithm
exchanges a bit of edge-search distance for correctness.
Figure~\ref{fig:rear_car} shows that the longer gradient of \texttt{fxaa3d}
has a negligible effect.

Finally, Figure~\ref{fig:small_structures} shows that the proposed algorithm avoids blurring away small/thin structures.
...
To measure the performance in terms of speed, benchmarks were performed on a
...
\begin{figure}[tb]
  ...
  \end{subfigure}
  \begin{subfigure}[b]{0.4\textwidth}
    \includegraphics[width=\textwidth]{images/egypt.png}
    \caption{egypt.png}
  \end{subfigure}
...
It performs consistently faster than the blur filter, which is surprising because the latter takes only 9 texture samples. This
illustrates that SSAA with a factor 2 is a feasible method. Note that the costs
for rendering the frame at a higher resolution are not included in the
benchmarks, but these costs would be the same as when a HiDPI screen would have been used.
...
faster than the algorithms that apply an edge-search. They can be faster than
...
In Intel integrated graphics and Apple Silicon, DDAA performs consistently
...
and FXAA have similar performance.

For the dedicated graphics cards (all Nvidia), DDAA performs
either better or similar to FXAA, depending on the model.
