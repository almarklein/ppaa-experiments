...
shown in Figure~\ref{fig:near_hori}. Improvements to this algorithm have also
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=\textwidth]{images/near_hori.png}
  \caption{Comparison of different algorithms on an image containing near-horizontal lines.
    The algorithms that include edge-search (\texttt{fxaa3d} and \texttt{ddaa2}) produce a notably smoother result.
    Supersampling (\texttt{ssaax2}) does not help much to counter the staircase effect.}
  \label{fig:near_hori}
\end{figure}
...
\texttt{fxaa3d}. In Figure~\ref{fig:tick_mark} the gradients
...
Figure~\ref{fig:near_hori} shows that the longer gradient of \texttt{fxaa3d}
